...
class _NNBase(BaseEstimator, ABC):
    ...
    def _build_problem_and_fitness_function(
        X, y, node_list, activation, learning_rate, bias, clip_max, is_classifier=True
    ):
        # Initialize optimization problem
        fitness = NetworkWeights(
            X, y, node_list, activation, bias, is_classifier, learning_rate=learning_rate
        )
    ...
    @staticmethod
    def _predict(
        X, fitted_weights, node_list, bias, input_activation, output_activation, is_classifier=True
    ):
